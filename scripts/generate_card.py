"""
Gera um card SVG de estatísticas do GitHub para o README.
...
Variáveis:
    ...
    GH_USERNAME  - nome do usuário (opcional; padrão: joao-torre)
...
if not TOKEN:
    raise RuntimeError("GITHUB_TOKEN não foi definido.")
...
def fetch_overview():
    ...
    result = gql(query, {"login": USERNAME})
    return result["user"]
...
def compute_streaks(weeks):
    ...
    by_date = {
        datetime.date.fromisoformat(date): count
...
def top_languages(repo_nodes, limit=4):
    ...
    for repo in repo_nodes:
        ...
        colors[name] = language.get("color") or "#94A3B8"
    ...
    return [
        (name, count / total * 100, colors[name])
...
def render_svg(stats):
    ...
    for name, pct, color in languages:
        width = 270 * (pct / 100)
        bar_segments.append(
            f'<rect x="{x:.1f}" y="50" width="{width:.1f}" '
            f'height="14" fill="{escape(color)}"/>'
        )
        x += width
    ...
    for (lx, ly), (name, pct, color) in zip(
        legend_positions, languages
    ):
        legend.append(
    ...
    circumference = 389.5
    streak_ratio = min(stats["current_streak"] / 30, 1.0)
  ...
  <rect x="1" y="1" width="718" height="458" rx="18"
        fill="url(#bg)" stroke="#071A35" stroke-width="1.5"/>

  <rect x="1" y="1" width="718" height="458" rx="18"
        fill="none" stroke="url(#accent)" stroke-width="1" opacity="0.35"/>

  <text x="32" y="46" class="title">{escape(stats["name"])}</text>
  <text x="32" y="66" class="subtitle">{escape(stats["subtitle"])}</text>

  <line x1="32" y1="82" x2="688" y2="82"
        stroke="#071A35" stroke-width="1"/>

  <g>
    <rect x="32" y="102" width="200" height="92" rx="12"
          fill="#071A35" stroke="#071A35"/>

    <rect x="248" y="102" width="200" height="92" rx="12"
          fill="#071A35" stroke="#071A35"/>

    <rect x="464" y="102" width="224" height="92" rx="12"
          fill="#071A35" stroke="#071A35"/>
  ...
  <g transform="translate(32,214)">
    <rect x="0" y="0" width="330" height="220" rx="14"
          fill="#071A35" stroke="#071A35"/>

    <text x="20" y="30" class="sectionHead">
      Streak de contribuições
    </text>

    <g transform="translate(90,60)">
      <circle cx="75" cy="75" r="62"
              fill="none" stroke="#020617" stroke-width="10"/>

      <circle cx="75" cy="75" r="62"
              fill="none" stroke="url(#accent)" stroke-width="10"
      ...
      <text x="75" y="70" text-anchor="middle" class="streakNum">
        {stats["current_streak"]}
      </text>

      <text x="75" y="92" text-anchor="middle" class="streakLbl">
        dias seguidos
      </text>
    </g>

    <text x="20" y="200" class="langPct">
      Maior streak: {stats["longest_streak"]} dias
    </text>
  </g>

  <g transform="translate(378,214)">
    <rect x="0" y="0" width="310" height="220" rx="14"
          fill="#071A35" stroke="#071A35"/>
    ...
    <rect x="20" y="50" width="270" height="14" rx="7"
          fill="#020617"/>
  ...
  <text x="688" y="446" text-anchor="end" class="langPct" opacity="0.6">
    github.com/{escape(stats["username"])}
  </text>
</svg>"""
...
def main():
    ...
    with open("generated/card.svg", "w", encoding="utf-8") as file:
        file.write(render_svg(stats))
